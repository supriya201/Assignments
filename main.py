...
assignment.date_format(data,"Month and Year")

assignment.sumOfNum(data,"Numbers")

assignment.cummmu_sum(data,"Numbers")
assignment.deleteValues(data,"FormID")
assignment.deleteValues(data,"FormSubmissionID")
assignment.concat_values(data,"Category","Sub-Category")
assignment.type_entry(data,"Type","Actual")
assignment.type_entry(data,"Type","Forecast")
print assignment.get_number(data)

for row in data:
    date1 = datetime.date(2020,01,01)
    date2 = (datetime.datetime.strptime(row["Month and Year"],"%m\/%d\/%Y")).date()
    assignment.no_of_days(date1, date2)
